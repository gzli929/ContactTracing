...
def DegGreedy_private_V2(state: InfectionState, epsilon: float):
    ...
    for u in state.V1:
        ...
        infected_nbr_count = sum([1 for v in set(state.G.neighbors(u)) if v in I2])
...
'''def Degree_I_noisy(state: InfectionState, epsilon: float):
    ...
    degrees: List[Tuple[int, int]] = []
    for u in state.V1:
        count = sum([1 for v in state.G.neighbors(u) if v in I2])
        degrees.append((max(1, count+ sample_dgauss(1/epsilon)), u))
...
config = {
    ...
    #"budget": [i for i in range(2000, math.floor(len(G.nodes)*0.05), int((math.floor(len(G.nodes)*0.05)-2000)/20))],
    "policy": ["none"],
    "transmission_rate": [0.05],
    "transmission_known": [True],
