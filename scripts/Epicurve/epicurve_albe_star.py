...
def run_full():
    ...
    for agent in [DegGreedy_fair, DepRound_fair, EC]:
        ...
        with open(PROJECT_ROOT/"data/SIR_Cache/albe_star.json", 'r') as infile:
            j = json.load(infile)
        ...
        state = InfectionState(G, (S, I1, I2, R), 1350, "none", 0.05, True, -1, True, 1)
...
def run_manual():
    ...
    for agent in [SegDegree, Random]:
        ...
        state = InfectionState(G, (S, I1, I2, R), 1350, "none", 0.05, False, -1, False, 1)
...
def run_digital():
    ...
    for agent in [DegGreedy_private_V2, Degree_V2_noisy]:
        ...
        state = InfectionState(G, (S, I1, I2, R), 2700, "none", 0.05, False, compliance_avg, False, 1)
...
for l in I_manual:
     if len(l)<max_timestep:
            l += [0]*(max_timestep-len(l))
...
with open(PROJECT_ROOT/"output"/"albestar_epicurve.json", 'w') as f:
